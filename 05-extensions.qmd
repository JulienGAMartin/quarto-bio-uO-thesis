...
\noindent This chapter describes a number of additional tips and tricks as well as possible customizations to the `bio_uO_down` thesis.
...
If you don't use caching and you would like to just have the **_bookdown_files** folder deleted after the build process is complete, then set `allow_cache = FALSE` in **index.qmd**'s call to `knit_thesis`.
...
To turn off correction highlighting, go to the YAML header of **index.qmd**, then:
...
You can add a second abstract in **index.qmd** like so:
...
Therefore, when you build your thesis to PDF, split the PDF into an alphanumerically sorted sequence of **single-page** PDF files (you can do this automatically with the package `pdftools`). You can then use the appropriate LaTeX command to insert them, as shown below (for brevity, in the `bio_uO_down` PDF sample content we're only including two pages).
...
# 1.0 is 100 per cent of the bio_uO_down PDF page width;
...
## Including another paper in your thesis - R Markdown child document {#embed-qmd}
...
Take this simple example (files for this are in [this GitHub repository](https://github.com/ulyngs/bio_uO_down-external-article)):
...
In the **paper_to_include** folder, the file **my_paper.qmd** is where you write the paper.
In **my_paper.qmd**, you read in a CSV file found in the subfolder **data/cats.csv**, and also an image from the subfolder **figures/cat.jpg**.

### Step 1: Include paper as a child document
In your thesis folder, create an qmd file for the chapter where you want to include another paper. 
...
`r ''````{r child = "../paper_to_include/my_paper.qmd"}
```
````
...
In **my_paper.qmd**, create a parameter called `other_path` and set it to an empty string:
...
In **my_paper.qmd**, put this at the start of the filepath when you read in data or include images:
...
Finally, in your thesis folder's **index.qmd** file, also create the parameter `other_path`.
...
To avoid this, first _increment all heading levels by one in **paper_to_include/my_paper.qmd**_ (\# Some header -> \#\# Some header).
...
In the YAML header of **paper_to_include/my_paper.qmd**, use this filter:
...
In **paper_to_include/my_paper.qmd**, first add a parameter we could call `out_width` and set it to the string "80%":
...
Then, make sure use that parameter to set the output width when you include figures in **paper_to_include/my_paper.qmd**:
...
Finally, create the parameter `out_width` in your thesis' **index.qmd** file:
...
To use [biblatex](https://www.overleaf.com/learn/latex/Bibliography_management_with_biblatex) to handle citations, first uncomment this in **index.qmd**, YAML header:
...
Second, set the parameter `insertHeadingInPDF: false` in **index.qmd**, to suppress the inclusion of a 'References' heading at the end of the thesis.
...
To use [natbib](https://www.overleaf.com/learn/latex/Bibliography_management_with_natbib) to handle citations, first uncomment this in **index.qmd**, YAML header:
...
This can now be done directly in **index.qmd**'s YAML header.
...
At the time of writing, you must set this directly in **template.tex** rather than from the YAML header in **index.qmd**.


## Customising to a different university
### The minimal route
If the front matter in the OxThesis LaTeX template is suitable to your university, customising `bio_uO_down` to your needs could be as simple as putting the name of your institution and the path to your university's logo in **index.qmd**:
...
If you have a **.tex** file with some required front matter from your university that you want to replace the OxThesis template's title page altogether, you can provide a filepath to this file in **index.qmd**.
`bio_uO_down`'s sample content includes and example of this --- if you use the YAML below, your front matter will look like this:
